...
import {ISecretariaCreateDB} from "../../../info/models/ISecretariaCreateDB.ts";
...
export default function SecretariaForm(){
    ...
    const { register, handleSubmit } = useForm<ISecretariaCreateDB>();
    const { setShowModal } = useContext(UsuariosContext);
    const {createSecretaria} = useApiSecretaria()
    const {getUsuarios} = useApiUsuarios()
    const isLoading = false;

    const onSubmit: SubmitHandler<ISecretariaCreateDB> = (data) => {
